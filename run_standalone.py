...
def check_gmm_vs_ragged_dot(lhs, rhs, group_sizes, tiling, atol=5e-2):
    ...
    print(f"shape megablox out = {out_gmm.shape}")
    print(f"shape out_ragged out = {out_ragged.shape}")
    print(f"out_gmm: {out_gmm}")
    print(f"out_ragged: {out_ragged}")
    print(f"diff: {out_gmm - out_ragged}")
...
def test_gpu_megablox():
    ...
    #rhs = jax.random.normal(key, (7, K, N))
    #tiling = (64, 64, 64)
    ...
    tile_size = (128, 128, 128)  # no partial leftover
    print("\nTest 2:")
    check_gmm_vs_ragged_dot(lhs, rhs, group_sizes, tile_size, 1e-3)
