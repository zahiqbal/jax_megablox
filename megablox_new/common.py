...
def is_gpu() -> bool:
  return "gpu" in jax.devices()[0].platform
...
_TPU_KIND_PATTERN = re.compile(r"gpu v(\d+)")


def gpu_generation() -> int:
  """Generation number of the currently attached TPU."""
  if version := _TPU_KIND_PATTERN.match(gpu_kind()):
    return int(version)
  raise NotImplementedError("only GPU devices are supported")


def supports_bfloat16_matmul() -> bool:
  """Does the currently attached CPU support bfloat16 inputs?"""
  return is_gpu() # ensure that its MI300 onward ...
