...
def _gmm(
  ...
  def kernel(
    ...
    def _accum(is_last_k_tile):
      ...
      loaded_lhs = pl.load(lhs, (pl.ds(m_tile_ids[grid_id]*tm, tm), pl.ds(k_i*tk, tk)) , mask=_lhs_grp_mask_ )

      g_index = (group_ids[grid_id]-group_offset[0])
      if transpose_rhs:
        loaded_rhs = pl.load(rhs, (g_index, pl.ds(n_i*tn, tn), pl.ds(k_i*tk, tk))  )
      else:
        loaded_rhs = pl.load(rhs, (g_index, pl.ds(k_i*tk, tk), pl.ds(n_i*tn, tn))  )
      ...
      pl.atomic_add(out, (pl.ds(m_tile_ids[grid_id]*tm, tm), pl.ds(n_i*tn, tn)) , block_result, mask=_store_mask_ )
